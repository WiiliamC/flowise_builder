...
interface Rule { alias: string; field: string; type: 'boolean' | 'number' | 'options'; min?: number; max?: number; integer?: boolean; values?: string[] }
...
function state(config: Record<string, unknown>, a: Allowed) {
  ...
  return valid(value, a) ? { state: 'stored' as const, value } : { state: 'invalid' as const }
}
async function context(client: Client, targetId: string, agentRef: string) {
  ...
  try { schema = await client.getNode('chatOpenAI') } catch { return fail('AGENT_MODEL_CATALOG_INVALID', 'Model catalog could not be read') }
  if (!schema || schema.name !== component) fail('AGENT_MODEL_CATALOG_INVALID', 'Model catalog does not match the component')
  return { remote, flow, config, component: 'chatOpenAI', allowed: intersect(rules, schema), supported: true }
}
export async function inspectAgentModel(client: Client, input: { targetId: string; agentRef: string }) {
  ...
  return { agentRef: input.agentRef, updatedDate: c.remote.updatedDate, component: c.component, supported: c.supported, warnings: [warning], parameters: c.allowed.map((a) => ({
    alias: a.rule.alias, type: a.rule.type, ...(a.values ? { enum: a.values } : {}), ...(a.min !== undefined ? { min: a.min } : {}), ...(a.max !== undefined ? { max: a.max } : {}),
    dependencies: a.rule.field === 'reasoningEffort' ? { 'reasoning.enabled': true } : {},
    catalogVisibilityConditional: Boolean(a.param.show || a.param.hide), stored: state(c.config, a),
    catalogDefault: a.param.default === undefined ? { state: 'absent' } : valid(a.param.default, a) ? { state: 'available', value: a.param.default } : { state: 'invalid' }
  })) }
}
export async function editAgentModel(client: Client, input: { targetId: string; agentRef: string; ifMatchUpdatedAt: string; set: string[]; apply: boolean }) {
  if (!input.ifMatchUpdatedAt) fail('UPDATED_DATE_REQUIRED', 'An updatedDate match is required')
  if (!input.set.length) fail('AGENT_MODEL_PARAM_INVALID', 'At least one parameter assignment is required')
  const c = await context(client, input.targetId, input.agentRef)
  ...
  for (const assignment of input.set) {
    ...
    const alias = assignment.slice(0, index)
    const a = c.allowed.find((item) => item.rule.alias === alias)
    if (index < 1 || !a || assignments.has(a)) fail('AGENT_MODEL_PARAM_INVALID', 'Parameter is unknown, unavailable, or duplicated')
  ...
  for (const [a, value] of assignments) config[a.rule.field] = value
  for (const [a] of assignments) {
    if (a.rule.field === 'reasoningEffort' && config.reasoning !== true) fail('AGENT_MODEL_DEPENDENCY_INVALID', 'Reasoning effort requires reasoning to be explicitly enabled')
  ...
  const changes = [...assignments].filter(([a, value]) => !isDeepStrictEqual(c.config[a.rule.field], value)).map(([a]) => ({ alias: a.rule.alias, before: state(c.config, a), after: state(config, a) }))
