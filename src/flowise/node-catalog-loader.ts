...
export async function snapshotCatalog(client: FlowiseClient, path: string, selectedNodes?: NodeDataSchema[]) {
  const nodes = selectedNodes ?? await client.listNodes(); const source = createHash('sha256').update(client.baseUrl).digest('hex').slice(0, 12); const snapshot: CatalogSnapshot = { schemaVersion: '1', fetchedAt: new Date().toISOString(), source: `instance:${source}`, schemaHash: catalogHash(nodes), nodes }
