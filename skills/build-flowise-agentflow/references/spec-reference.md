...
Use `flowise-agentflow list --format json` and `flowise-agentflow inspect --target-id ID --format json` to discover existing workflows without exporting raw FlowData. Use `flowise-agentflow inspect-nodes --component NAME --format json` before choosing fields. FlowData artifacts can contain credential IDs and prompts; store them as sensitive files.
