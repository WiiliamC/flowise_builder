...
Before update, always diff. Do not use `--force` without explicit authorization. Create, copy, and update require explicit user authorization plus `--apply --format json`. Copy warnings require explicit `--allow-warnings`; errors always block. After create or update, inspect the remote flow. Copy performs its own remote readback and semantic persistence check, so do not add a separate inspect unless the user asks for structural details. Export and validate only when a protected raw verification artifact is required. Report the chatflow ID, semantic changes, warnings, and manual checks without secrets.
