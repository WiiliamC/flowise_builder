...
Use only the CLI for Flowise discovery and inspection. Do not call the Flowise API directly, write an ad hoc FlowData parser, guess an ID, or expose raw FlowData merely to summarize a workflow. To duplicate an existing canvas in the same instance, use `copy --source-id ID --name NAME`; it validates the live canvas and does not require YAML or export artifacts. To rename an existing workflow, use `rename --target-id ID --name NAME --format json`; no spec, export, or catalog is needed. Preview first, then add `--apply` when authorized. The optional `--if-match-updated-at DATE` checks the exact inspected timestamp, even for a no-op. Treat runtime flow IDs and names as local operational data and never put captured output in repository artifacts. Use `export` only when a protected raw artifact is explicitly needed.
...
| `rename` | None | Default/dry run: none. With explicit authorization and `--apply`: PUTs only `{ "name": NAME }` to the target `AGENTFLOW`; rereads type/name/timestamp before writing and verifies persisted ID/type/name afterward. Identical names never PUT. |
...
| `edit-agent-mcp` | None, except it reads a caller-provided UTF-8 strict-JSON config file | Default/dry run: none. With explicit authorization and `--apply`: replaces only one existing Custom MCP's `mcpServerConfig` after exact timestamp and full-FlowData concurrency checks; it never discovers or changes actions. |
| `refresh-agent-mcp-actions` | None | Always asks Flowise to discover current actions from the stored config. Refresh-only never writes. Only explicit additive enablement plus `--apply` can PUT, after exact timestamp and full-FlowData concurrency checks. |

Before update, always diff. Do not use `--force` without explicit authorization. Create, copy, rename, update, edit-system-prompt, edit-agent-mcp, edit-agent-model, and MCP action enablement require explicit user authorization plus `--apply --format json`. For Custom MCP work, run `inspect-agent-mcp`, then optionally `edit-agent-mcp`, then `refresh-agent-mcp-actions`; never place the config or discovered descriptions in a report. Use `--show-action-names` only when needed. V1 cannot add, delete, or reorder Custom MCP tools or disable actions, and it never bypasses Flowise authentication or target policy. Copy warnings require explicit `--allow-warnings`; errors always block. After create or update, inspect the remote flow. Copy and precise editors perform their own remote readback and semantic persistence check, so do not add a separate inspect unless the user asks for structural details. Export and validate only when a protected raw verification artifact is required. Report the chatflow ID, semantic changes, warnings, and manual checks without secrets.

Rename rejects whitespace-only names and preserves other names exactly. Its timestamp check is optional and client-side; there is still a race between the immediate reread and PUT because the server does not enforce an atomic precondition. Rename performs its own name readback check and never retries uncertain writes or falls back to a full canvas update. See `references/workflow-patterns.md` for the rename sequence.
